# Juno OS — Deal Page Restructure Specification (v3.7)
...
**Version:** 3.7
**Date:** 15 May 2026
**Status:** Phase A1 closed — Stages 1–7 complete. Stage 6d (client-section proof-of-concept) deferred to start of Phase B (client record). Phase A2 (sell deal redesign + Phase A1 cleanup absorption) sits in Future Work 14.1.
**Supersedes:** v1, v2, v3, v3.1, v3.2, v3.3, v3.4, v3.5, v3.6

**What's new in v3.7:**

Stage 7 — Phase A1 closure. Stage 7 was originally scoped as a cutover with feature-flag flip, component archival, and `deal_investors.amount` column drop. The actual build of Stages 1–6c never used a feature flag; the buy-deal cutover happened quietly during Stage 2 when `page.tsx` was wired to route by `deal_type`. Stage 7 is therefore not a cutover but a formal closure of Phase A1, with three pieces of cleanup explicitly deferred to Phase A2: dropping the `deal_investors.amount` column (still written by both buy and sell creation paths), hard-deletion of the legacy `DealDetail.tsx` tree (still in production because sell deals render it), and stripping dead buy-deal code paths from `EditInvestorsClient.tsx`. Future Work 14.1 is retitled and expanded as "Phase A2 — sell deal redesign and Phase A1 cleanup absorption" to combine the sell-side redesign with the deferred cleanup into a single block of work. Section 11.4's stale "uses jsPDF" line for transaction statements is corrected to point at the React-pdf pipeline introduced in Stage 6c. A note is added at the top of Section 14 explaining that the registry's numbering is intentionally non-contiguous (specifically, there is no 14.12), to preserve the integrity of older changelog entries.

Two small housekeeping items also closed in this v3.7 work, separate from the spec edits:

- Future Work 14.8 (`team_members` backfill) — done. Migration `team_members_backfill` inserted rows for Edward Rudd (ER) and Julian Hickman (JH), and added a trigger on `auth.users` to auto-create future `team_members` rows. Follow-up migration `handle_new_auth_user_revoke_execute` locked down the function's REST API surface to address the security-definer-function advisor warning introduced by the trigger.
- Future Work 14.17 (platform-wide storage key sanitisation policy) — deliberately deferred to the start of Stage 6d, with a v3.7 working leaning toward a lightweight typed wrapper (`uploadDocument()` helper). Decision recorded inline in 14.17 so the design conversation is half-done when the work begins. Re-evaluate against Stage 6d's concrete second consumer at that time.
...
**[CORRECTED IN v3.7]** Generate transaction statement uses the React-pdf pipeline introduced in Stage 6c (`transactionStatement@1.0.0` via the dedicated `generateTransactionStatement()` function — see the Stage 6c subsection of the Document Generation Architecture below for detail). It is real, not mock. The legacy jsPDF generator at `src/lib/services/statementGenerator.ts` still exists, marked `LEGACY` and imported only by `InvestmentCockpit.tsx`; removal is tracked in Future Work 14.16.
...
### Stage 7 — Phase A1 closure (1 day, spec-only)

**[REWRITTEN IN v3.7 — based on actual build state]**

Stage 7 was originally scoped as a cutover with feature-flag flip, component archival, and `deal_investors.amount` column drop. The actual build of Stages 1–6c did not use a feature flag; the buy-deal cutover happened quietly during Stage 2 when `page.tsx` was wired to detect `deal_type` and route `new_investment`/`follow_on` deals to the new `BuyDealPage.tsx`. As a result Stage 7 is no longer a cutover — it is a formal closure of Phase A1 with most cleanup deferred to Phase A2 (see Future Work 14.1).

**In scope for Stage 7:**

1. Update this spec to record what was actually built and what is deferred (this section, the rewritten Future Work 14.1, the corrected Section 11.4 jsPDF reference, and the version history).
2. Confirm the live state of the routing in production: `src/app/(app)/deals/[id]/page.tsx` routes by `deal_type` — buy deals (`new_investment`, `follow_on`) to `BuyDealPage.tsx`, all other types (sell, exit, etc.) to the legacy `DealDetail.tsx`.

**Deliberately deferred to Phase A2 (Future Work 14.1):**

1. Dropping the `deal_investors.amount` column. The column is still written by both buy and sell deal creation paths (`src/app/(app)/deals/new/buy/InvestorsStep.tsx`, `src/app/(app)/deals/new/sell/InvestorsStep.tsx`, `src/app/(app)/deals/[id]/AddInvestorsModal.tsx`, `src/app/(app)/deals/new/NewDealWizard.tsx`, and `bookbuildSideEffects.ts`). Updating sell-deal code paths to stop writing it is work that fits naturally into Phase A2, when those paths are being redesigned anyway. Stage 7 leaves the column in place; the new buy-deal page reads from `soft_circle_amount` and `confirmed_amount`, not `amount`.
2. Hard-deletion of the legacy `DealDetail.tsx` tree (`DealDetail.tsx`, `BookbuildSection.tsx`, `BookbuildEntryPopover.tsx`, `AddBookbuildEntryModal.tsx`, `bookbuildSideEffects.ts`, `PreCloseTab.tsx`, `PostDealTab.tsx`, `CompletionChecklist.tsx`, `GenericChecklist.tsx`, `SignatureTracking.tsx`, `TrancheSchedule.tsx`, `dealDetailTypes.ts`). These remain in production because sell deals still render them. They can only be removed once sell deals have been migrated to a new tabbed implementation in Phase A2.
3. Stripping dead buy-deal code paths from `src/app/(app)/deals/[id]/edit/EditInvestorsClient.tsx`. The buy-deal branches are unreachable in production (buy deals already redirect away from this page), but the file is still actively used by sell deals via the `dealTypeCategory="sell"` branch. Removing the dead buy branches is a small clean-up that fits Phase A2.

**Not in scope (permanently or separately):**

- Data wipe / importer-driven reload. Handled separately as Ed's Phase E whenever importer work happens.
- Any change to `BuyDealPage.tsx` or its tab components — Phase A1 has shipped and no functional changes are part of Stage 7.
- Any database migration — Stage 7 does not touch the database.
...
**[v3.7 note]** The numbering of items in this registry is intentionally non-contiguous (specifically, there is no 14.12). Items are assigned numbers when added across spec versions, and a closed or removed item does not cause downstream items to be renumbered — this preserves the integrity of older changelog entries and external references. The presence of gaps is a feature, not a typo.

### 14.1 Phase A2 — sell deal redesign and Phase A1 cleanup absorption

**[RETITLED AND EXPANDED IN v3.7]** Phase A2 is the next major block of deal-page work. It combines two things that happen naturally together: redesigning the sell-side of the deal page using the patterns established by Phase A1, and absorbing the cleanup that Stage 7 deliberately deferred. Both touch the same legacy code (`DealDetail.tsx` and its tree); doing them in the same phase avoids opening the same files twice.

#### 14.1.1 Sell deal page redesign

The new tabbed page applies to buy deals only. Sell deals continue using the existing `DealDetail.tsx` implementation until Phase A2.

A future project will redesign the sell deal page using the same tabbed pattern as buy deals, adapted for sell-specific flow: FIFO lot matching, deferred consideration, tranche schedules, and statuses (selling / not selling / undecided).
...
The sell deal table should have a similar three-column structure for the seller identity, plus sell-specific columns (proceeds, current holding, FIFO lot mapping, etc.).

**[NEW IN v3]** Note: 16 sell deals exist as test data. The two routes will coexist briefly during the build, then the old route is removed once the new sell-deal page covers all the existing cases.

#### 14.1.2 Phase A1 cleanup absorption

Three pieces of work were deliberately deferred from Stage 7 to Phase A2 (see Section 12, Stage 7 for the reasoning):

1. **Drop the `deal_investors.amount` column.** The column is currently written by both buy and sell deal creation paths. As part of redesigning sell deals in Phase A2, the sell-side writes get migrated to `soft_circle_amount` / `confirmed_amount` (matching the buy-side model), the buy-side writes that still populate `amount` get removed (`src/app/(app)/deals/new/buy/InvestorsStep.tsx`, `src/app/(app)/deals/[id]/AddInvestorsModal.tsx`, `src/app/(app)/deals/new/NewDealWizard.tsx` if it still exists), and then the column is dropped via migration.
2. **Hard-delete the legacy `DealDetail.tsx` tree.** Once the new sell-deal page replaces it, the entire legacy tree can be removed: `DealDetail.tsx`, `BookbuildSection.tsx`, `BookbuildEntryPopover.tsx`, `AddBookbuildEntryModal.tsx`, `bookbuildSideEffects.ts`, `PreCloseTab.tsx`, `PostDealTab.tsx`, `CompletionChecklist.tsx`, `GenericChecklist.tsx`, `SignatureTracking.tsx`, `TrancheSchedule.tsx`, `dealDetailTypes.ts`, plus the `/deals/[id]/edit/` route and its `EditInvestorsClient.tsx` (since the buy-deal branch is dead and the sell-deal branch is replaced by the new tabbed page's modal).
3. **Update the routing in `page.tsx`.** Once sell deals have their own dedicated page (e.g. `SellDealPage.tsx`), the legacy fallback in `page.tsx` can be removed, leaving a clean per-deal-type router with no legacy branch.

**Trigger to start:** when sell deal volume justifies the redesign work, or when inconsistency between buy and sell becomes painful. The cleanup items in 14.1.2 are not the trigger by themselves — the sell-deal redesign drives the timing, and the cleanup rides along.

**Sequencing within Phase A2:** sell-deal redesign first (Stages A2.1–A2.n, mirroring Phase A1's stage structure), then cleanup (final stage of Phase A2). This avoids deleting code that the new sell-deal page might still want to learn from.
...
**Trigger to start:** at the start of Stage 6d (the next new document type — engagement letter or welcome pack). That's the natural moment because Stage 6d is the first new document consumer since this question was raised, so the wrapper can be designed against two concrete users (the existing application form / transaction statement paths and the new client-section path) rather than guessed at against one.

**[NOTED IN v3.7 — working leaning]** When this work happens, the spec's current leaning is toward a **lightweight typed wrapper** (a fourth option, sitting between the typed wrapper and convention-only options above). Specifically: a small `uploadDocument()` function in `src/services/document-generation/storage.ts` that takes the human-facing filename and applies `sanitiseStorageKey()` internally. Existing call sites get migrated mechanically; AGENTS.md §2.2 gets updated to name `uploadDocument()` as the canonical entry point. This was chosen over the lint-rule option because the codebase has no existing custom-ESLint-rule infrastructure (standing it up for one rule is half-to-full-day of work), and over the convention-only option because that has already failed once (the Stage 6c bug). The wrapper is roughly 20 lines of code; the migration of existing call sites is mechanical. A lint rule can be layered on top later if belt-and-braces becomes worth it. This leaning is not a commitment — when Stage 6d starts, re-evaluate against the second consumer's needs and decide finally then.
...
- **v3.7 hotfix 2 (29 May 2026)** — Two buy-deal fixes and bank details implementation. (1) Fractional shares: `InvestorsStep.tsx` `computeRow` now applies `Math.round()` to `sharesNum`; because all downstream values (`shares_purchased`, `sum_subscribed`, `deal_investors.amount`) derive from this single variable, the fix is a single line and covers all code paths. (2) Bank details: `bank_account_name`, `bank_sort_code`, `bank_account_number`, `bank_iban`, `bank_swift_bic` added to both `companies` and `nominees` tables via migration `add_bank_details_to_companies_and_nominees` (these fields were designed in v3.4 but the migration and UI were never built). The `portfolio/[id]/settings` "Coming soon" stub replaced with a full `CompanySettingsForm` covering general info (name, sector, stage, website, description, EIS eligible) and bank details. The nominees add/edit modal extended with a bank details section; list cards show a "Bank details set" indicator. `src/types/index.ts` `Company` interface updated with the five new columns.

- **v3.7 hotfix (29 May 2026)** — Clients section: removed all references to the dropped `clients.fund_type` and `clients.entity_type` columns (removed in Entity Model Cleanup Sub-stage A, 23 May 2026) that had been missed when PR #21 fixed the equivalent issue in the deals section. Affected files: `clients/page.tsx` (listing query), `ClientList.tsx` (type, filter state, filter UI, fund-type pill), `clients/[id]/page.tsx` (group-members query), `clients/new/page.tsx` + `NewClientForm.tsx` (leads query, form state, INSERT payload, fund-type selector UI), `clients/[id]/edit/page.tsx` + `EditClientForm.tsx` (client query, form state, UPDATE payload, entity-type and fund-type selector UI). Root cause: PostgREST returns an error (not null) when a non-existent column is selected; the code discarded errors silently, causing the entire clients listing to render empty. The "Fund type" filter on the client list and "Fund type"/"Entity type" fields on the new/edit client forms have been removed; fund type is per-investment (`investments.fund_type`) and entity type is captured via the 3D investor identity model.

- **v3.7 (15 May 2026)** — Phase A1 closure (Stage 7) plus housekeeping. Stage 7 rewritten to reflect actual build state: no feature flag was used, cutover happened quietly during Stage 2, so Stage 7 is now formal closure rather than a real cutover. Three pieces of cleanup work explicitly deferred to Phase A2: dropping `deal_investors.amount`, hard-deletion of the legacy `DealDetail.tsx` tree, and stripping dead buy-deal code paths from `EditInvestorsClient.tsx`. Future Work 14.1 retitled and expanded as "Phase A2 — sell deal redesign and Phase A1 cleanup absorption", combining the sell-side redesign with the deferred cleanup into a single block of work. Stale jsPDF reference in Section 11.4 corrected to point at the React-pdf pipeline introduced in Stage 6c. Note added at top of Section 14 explaining intentional numbering gaps (specifically no 14.12) to preserve changelog integrity. Housekeeping: Future Work 14.8 (`team_members` backfill) completed via the `team_members_backfill` migration plus a `handle_new_auth_user_revoke_execute` follow-up that closed the security-definer-function advisor warning. Future Work 14.17 (storage key sanitisation policy) deferred to Stage 6d, with a working leaning towards a lightweight typed wrapper recorded inline.
